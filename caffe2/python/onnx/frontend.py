...
from onnx.helper import make_node, make_tensor, make_tensor_value_info, make_attribute, make_model
import numpy as np
...
class Caffe2Frontend(object):
    ...
    def _ssa_rewrite(cls, net, init_net, value_info):
	    ...
	    for name, verson in versioned_inputs:
	    	for arg in op.arg:
			if arg.strings:
		    		arg.strings[:] = [w.replace(name, ssa_name(name, verson)).encode("utf8") for w in arg.strings]
			for nop in arg.n.op:
